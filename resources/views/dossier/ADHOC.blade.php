  ...
  <div class="container box" style="width:1100px; margin-left:-2%;">
    <nav style="position:absolute;margin-top:1%;width:100%;">
                ...
                <ul class="ul">
                    ...
                    <li style="background-color:#5bc0de;text-aligne:center;"><a href="{{url('dossier/ADHOC')}}">ADHOC</a></li>
    ...
    <div class="panel-heading">
     ...
     <div class="container">
        ...
        <div class="row pull-right" style="margin-right: 10%;">
                <a href="{{ route('view-pdf-ADHOC') }}" type="button" class="btn btn-info">Telecharger PDF</a>
                <a href="{{ url('downloadExcelSearchADHOC/xlsx') }}"><button class="btn btn-info">Exporter</button></a>
        </div>
        <form action="/casesADHOC" method="GET">
            From:<input type="date" name="from" required>
            To:<span><input type="date" name="to" required>
            <button type="submit" class="btn btn-info btn-sm">Chercher</button></span>
            </a>
        </form>
        <div style="float:right">
            <form action="{{ url('importExcelADHOC') }}" class="form-horizontal" method="post" enctype="multipart/form-data">
                @csrf
     ...
     <div class="table-responsive">
      <table class="table table-striped table-bordered">
          <thead>
              ...
              @can('adminOragent_gu')
              <th  style="text-align:center" width="4%"><a href="{{ url('dossier/createADHOC') }}">
   ...
   @foreach($projets as $projet)
      <tr style="text-align:center;">
        <td>{{ $projet->num}}</td>
         ...
         <td>
            @can('adminOragent_gu')
            <a href="{{url('historique/'.$projet->id.'/captADHOC')}}">
        ...
        <td>
          ...
          <a href="{{url('commission/'.$projet->id.'/createADHOC')}}">
         <button type="submit" class="table-add glyphicon glyphicon-plus" style="color:blue;"></button></a>&nbsp;&nbsp;&nbsp;   &nbsp;&nbsp;
         @else
         {{$projet->type}}
         @endif </td>
        ...
        <td>  <a href=" {{url('dossier/'.$projet->id.'/details')}} "> <button type="submit" class="glyphicon glyphicon-info-sign" style="color:blue;"></button>
         ...
         @can('adminOragent_gu')
          <a href=" {{url('dossier/'.$projet->id.'/editADH')}} "> <button type="submit" class="glyphicon glyphicon-pencil" style="color:blue;"></button>
              </a>&nbsp; <form action=" {{url('dossier/'.$projet->id)}} " method="POST">
              {{ csrf_field() }}
              {{ method_field('DELETE') }}
        ...
        @if($projet->commission=='1')
                ...
                <p style="color:red">Encore de examiné</p>
                <p style="color:red">Encore de examiné</p>
                @else
                <p></p>
                @endif
                ...
                <p style="color:red">Encore de validé</p>
                @else
                <p></p>
                @endif
