  ...
  <div class="container box" style="width:1100px; margin-left:-2%;">
    <nav style="position:absolute;margin-top:1%;width:100%;">
          ...
          <ul class="ul">
              ...
              <li style="background-color:#5bc0de;text-aligne:center;"><a href="{{url('dossier/GPU')}}">GPU</a></li>
    ...
    <div class="panel-heading">
     ...
     <div class="container">
        ...
        <div class="row pull-right" style="margin-right: 10%;">
            <a href="{{ route('view-pdf-gpu') }}" type="button" class="btn btn-info">Telecharger PDF</a>
            <a href="{{ url('downloadExcelSearchGPU/xlsx') }}"><button class="btn btn-info">Exporter</button></a>
        </div>
        <form action="/casesGPU" method="GET">
            ...
            <div style="float:right">
                <form action="{{ url('importExcelGPU') }}" class="form-horizontal" method="post" enctype="multipart/form-data">
                    @csrf
     ...
     <div class="table-responsive">
      <table class="table table-striped table-bordered">
          <thead>
              ...
              @can('adminOragent_gu')
              <th  style="text-align:center" width="4%"><a href="{{ url('dossier/createGPU') }}">
      ...
      <tr style="text-align:center;">
        <td>{{ $projet->num }}</td>
         <td>@can('adminOragent_gu')
           <a href="{{url('historique/'.$projet->id.'/captGPU')}}">
        ...
        <td>
          ...
          <a href="{{url('commission/'.$projet->id.'/createGPU')}}">
         <button type="submit" class="table-add glyphicon glyphicon-plus" style="color:blue;"></button></a>&nbsp;&nbsp;&nbsp;   &nbsp;&nbsp;
         @else
         {{$projet->type}}
         @endif </td>
        ...
        <td> <a href=" {{url('dossier/'.$projet->id.'/details')}} "> <button type="submit" class="glyphicon glyphicon-info-sign" style="color:blue;"></button>
               ...
               @can('adminOragent_gu')
               <a href=" {{url('dossier/'.$projet->id.'/editGPU')}} "> <button type="submit" class="glyphicon glyphicon-pencil" style="color:blue;"></button>
              </a>&nbsp;
        ...
        <td>
                ...
                @can('adminOrarchi')
                <a href="{{url('ficheDeTaxe/'.$projet->id.'/captGPU')}}">Génerer</a><br>
                @endcan
                @elseif($projet->ex=='1')
                ...
                @can('adminOrval_gu')
                <a href="{{url('ficheDeTaxe/'.$projet->id.'/captGPU2')}}">Examiner</a><br>
                @endcan
                @elseif($projet->val=='1')
                ...
                @can('adminOrval_gu')
                <a href="{{url('ficheDeTaxe/'.$projet->id.'/captGPU3')}}">Valider</a><br>
                @endcan
                @elseif($projet->val =='1')
                <a target="_blank" href="{{url('projetGPU/'.$projet->id.'/pdf_GPU_FicheTaxe')}}">Imprimer</a>
                @endif
                </td>
                @else
                <td></td>
                ...
                <td>
                  ...
                  <a target="_blank" href="{{url('Versement/'.$projet->id.'/captGPU')}}">
